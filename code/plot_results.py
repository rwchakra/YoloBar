...
with torch.no_grad():
    for image, target in tqdm(val_set):
        ...
        pil_vis_results_gt.save(os.path.join(
            results_vis_dir, img_fname.split('.')[0] + "_gt.png"))
        ...
        if PLOT_PREDICTIONS:
            ...
            vis_results_pred = draw_results(
                image=image * 255, masks=pred_masks, bboxes=pred_bboxes, scores=pred_scores)
            ...
            pil_vis_results_pred.save(os.path.join(
                results_vis_dir, img_fname.split('.')[0] + "_pred.png"))
